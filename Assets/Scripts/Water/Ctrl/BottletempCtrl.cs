...
namespace Game.Water
{
    public class BottletempCtrl : MonoBehaviour, IController, ICanSendEvent
    {
        ...
        private void Start()
        {
            ...
            //初始化瓶子位置
            this.RegisterEvent<GameStartEvent>(e => { OnCancelSelect(); })
        ...
        public void Init(LevelCreateCtrl.BottleProperty property, int idx)
        {
            ...
            foreach (var blackwater in blackWaterGos) blackwater.SetActive(false);
            ...
            if (IsBlackBottle)
                ...
                for (var i = 0; i < property.BlackBottleList.Count; i++)
        ...
        /// </summary>
        /// <param name="isFirst"></param>
        /// <param name="NeedUpdateWaterPos"></param>
                ...
                switch (waterItems[i])
                {
                    ...
                    case WaterItem.Bomb:
                        if (bombCounts[i] != BOMBREMOVE_SIGN)
                            waterImg[i].bombCtrl.SetBomb(true,
                                (bombCounts[i] - LevelManager.Instance.moveNum).ToString(),
                                "idle");
                        break;
        ...
        */
        ...
        {
            if (isClearHide || isFreeze || isNearHide || limitColor != 0 || IsBlackBottle | (curtainHight > 0))
        ...
        public void MoveTo(BottletempCtrl other)
        {
        ...
        public void MoveToOtherAnim(BottletempCtrl other, int topIndex, int numWater, int useColor)
        {
        ...
        /// <returns></returns>
        private (Vector3 pos, string dir) GetMoveToPos(Transform thisBottle, Transform targetBottle,
            Transform moveToTram)
        {
            // 要用本地坐标取镜像在转为世界坐标
        ...
        public bool BottleHasItem()
        {
            ...
            for (var i = 0; i < waters.Count; i++)
            {
                ...
                // 普通水/特殊水/非可合成类道具
                if (_waterColor <= 1000 || _attr is RainBowWaterState ||
                    _attr.SpineType is EColorStateSpineType.EFlyBomb)
                {
                    _itemId = 0;
                    continue;
                }
        ...
        /// </summary>
        /// <param name="color"></param>
        public BottletempCtrl CheckRemoveOneColor(int color)
        {
            for (var i = 0; i < waters.Count; i++)
                if (waters[i] == color)
                    return this;
        ...
        // 先更新的炸弹，后倒的水
        public void UpdateBomb(BottletempCtrl bottleCtrl = null, bool Init = false)
        {
            var moveNum = LevelManager.Instance.moveNum;
            var haveBomb = false;
            var _flag = false;
