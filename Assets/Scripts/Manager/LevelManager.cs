...
namespace Game.Water
{
    ...
    public class LevelManager : MonoBehaviour, IController, ICanSendEvent
    {
        ...
        public void CheckGuideLevel(int levelId)
        {
            ...
            switch (levelId)
            {
                ...
                case (int)UIGuideLevel.UIGuideLevel2:
                    ...
                    break;
                case (int)UIGuideLevel.UIGuideLevelRemoveHide:
                ...
                case (int)UIGuideLevel.UIGuideLevelRemoveAll:
                    UIKit.OpenPanel<UIPaidItemsGuide>(UILevel.PopUI, new UIPaidItemsGuideData()
                    {
            ...
            // 新机制引导
            if (GameConst.GameplayTutorialInfo.TryGetValue(levelId,
                    out var info))
            {
        ...
        public void UseItem(int itemId, Transform fromTarget)
        {
            ...
            var _attr = _field?.GetCustomAttribute<WaterColorState>();

            // 清除色块
            if (_attr is ClearItemState _clearAttr)
            {
            ...
            // 改变颜色
            if (_attr is ChangeColorItemState _changeAttr)
            {
                int fromColor = 0;
