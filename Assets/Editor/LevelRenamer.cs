...
public class LevelRenamer : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label($"重命名规则: {originalPrefix}[1-100] → {newPrefix}[{120}-{220}]", EditorStyles.boldLabel);
        ...
        foreach (var guid in guids)
        {
            ...
            if (match.Success)
            {
                ...
                if (currentNumber >= startNumber && currentNumber <= endNumber)
                {
                    ...
                    Debug.Log($"将会重命名: {fileName} → {newName}");
                }
                else
                {
    ...
    private void ExecuteRename()
    {
        ...
        foreach (var operation in renameOperations)
        {
            ...
            {
                Debug.LogError($"无法重命名 {operation.OriginalName} → {operation.NewName}，目标文件已存在！");
                errorCount++;
                continue;
            }
            ...
            {
                Debug.Log($"重命名成功: {operation.OriginalName} → {operation.NewName}");
                successCount++;
            }
        }
